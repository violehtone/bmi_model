#!/usr/bin/env python3
from sklearn.experimental import enable_iterative_imputer # required for imputation to work
...
def save_model_on_disk(model, output_model_name):
    """
...
def preprocess_data(data: str, imputer: impute.IterativeImputer) -> pd.DataFrame:
    """
    Reads the input data (.tsv file) into a Pandas dataframe and performs imputation in case of missing values
    """
...
def build_model(training_data: str, output_model_name: str, imputer: impute.IterativeImputer) -> linear_model:
    """
    ...
    logger.info(f"Building a lasso regression model with 10-fold CrossValidation...")
    cv = model_selection.RepeatedKFold(n_splits=10, n_repeats=3, random_state=1)
    model = linear_model.LassoCV(alphas=np.arange(0.01, 1, 0.01), cv=cv, n_jobs=-1, tol=0.05)
    model.fit(X, y)
...
def main(training_data: str, test_data: str, output_model_name: str, validate: bool):
...
def parse_args():
    """
    Parses the input arguments. This model building script requires two .tsv files as its input. One for training the
    lasso regression model and another one for optimizing it.
    """
